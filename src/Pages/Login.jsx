...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://crm-backend-if6g.onrender.com/api/auth/login",
        { email, password }
      );
      setLoading(false);
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "url(https://cff2.earth.com/uploads/2017/05/03154453/shutterstock_559454563.jpg)",
      ...
      <div className="w-full max-w-md p-8 space-y-8 bg-black bg-opacity-60 rounded-lg shadow-lg ">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              type="submit"
              className="w-full px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 flex justify-center items-center"
            >
              {loading ? (
